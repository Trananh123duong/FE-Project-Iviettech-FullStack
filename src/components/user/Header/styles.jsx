import bgHeader from '@assets/bg_header.jpg'
import styled from 'styled-components'

export const HeaderContainer = styled.header`
  z-index: 1000;
  width: 100%;
  background: url(${bgHeader}) top center repeat-x;
  box-shadow: 0 1px 0 rgba(0, 0, 0, 0.12);
`

export const Inner = styled.div`
  max-width: 1100px;
  margin: 0 auto;
  height: 56px;
  padding: 0 12px;

  display: grid;
  grid-template-columns: 220px 1fr auto;
  grid-template-areas: "logo search right";
  align-items: center;
  column-gap: 12px;

  /* đảm bảo co giãn đúng */
  min-width: 0;

  /* map area */
  & > .logo {
    grid-area: logo;
    min-width: 0;
  }
  & > .search-wrap {
    grid-area: search;
    min-width: 0;
  }
  & > .right {
    grid-area: right;
    min-width: 0;
  }

  /* ---------------------- Logo ---------------------- */
  .logo img {
    height: 36px;
    display: block;
    object-fit: contain;
  }

  /* ---------------------- Search ---------------------- */
  .search-wrap {
    position: relative;
    display: block;
  }

  .nt-input.ant-input {
    height: 40px;
    border-radius: 999px;
    padding: 0 48px 0 16px;
    border: 0;
    box-shadow: 0 2px 6px rgba(0,0,0,.08);
  }
  .nt-input.ant-input:focus {
    box-shadow: 0 0 0 2px rgba(47,111,234,.15), 0 2px 6px rgba(0,0,0,.08);
  }

  .nt-search-btn {
    position: absolute;
    top: 50%; right: 2px; transform: translateY(-50%);
    height: 36px; width: 42px;
    border: none; border-radius: 999px;
    background: #2f6fea; color: #fff; cursor: pointer;
    display: inline-flex; align-items: center; justify-content: center;
  }
  .nt-search-btn:hover { filter: brightness(1.05); }
  .nt-search-btn .anticon { font-size: 18px; }

  /* ---------------------- Actions ---------------------- */
  .right {
    display: inline-flex;
    align-items: center;
    gap: 8px;
    white-space: nowrap;
  }
  .icon-btn {
    color: #fff;
    width: 32px; height: 32px; padding: 0;
    border-radius: 8px; flex: 0 0 auto;
  }
  .icon-btn .anticon { font-size: 16px; }
  .icon-btn:hover { background: rgba(255,255,255,.14); }

  .account-btn {
    display: inline-flex;
    align-items: center;
    gap: 8px;
    height: 32px;
    padding: 0 8px;
    border: none;
    background: transparent;
    color: #fff;
    border-radius: 10px; cursor: pointer;
    flex: 0 0 auto;
  }
  .account-btn:hover {
    background: rgba(255,255,255,.14);
  }
  .account-btn .name {
    max-width: 150px;
    font-weight: 600;
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
  }
  .account-btn .caret {
    font-size: 12px;
    opacity: .85;
  }

  .account-menu .ant-dropdown-menu {
    min-width: 240px;
    border-radius: 10px;
    padding: 8px 0;
    box-shadow: 0 8px 26px rgba(0,0,0,.18);
  }
  .ant-input { border-color: transparent; }

  /* ---------- Responsive ---------- */

  @media (max-width: 900px) {
    .account-btn .name {
      display: none;
    }
  }

  @media (max-width: 768px) {
    height: auto;
    grid-template-columns: 1fr auto;
    grid-template-rows: 52px auto;
    grid-template-areas:
      "logo right"
      "search search";
    row-gap: 4px;
    padding: 6px 10px 6px;

    .logo img {
      height: 34px;
    }

    .nt-input.ant-input {
      height: 36px;
      padding-right: 44px;
    }
    .nt-search-btn {
      height: 32px;
      width: 38px;
      right: 3px;
    }
  }

  @media (max-width: 480px) {
    .icon-btn {
      width: 30px;
      height: 30px;
    }
    .account-btn {
      padding: 0 6px;
      height: 30px;
    }
    .nt-input.ant-input {
      height: 34px;
    }
    .nt-search-btn {
      height: 30px;
      width: 36px;
    }
  }
`
